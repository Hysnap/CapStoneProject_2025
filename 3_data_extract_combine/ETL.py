...
def extract_locations(text):
    logging.info("Starting location extraction from text...")
    # Load the unique locations data
    df_unique_locations = pd.read_csv('data/unique_locations.csv')
    logging.info("Unique locations data loaded.")

    # Exclude locations marked with ignore flag
    df_unique_locations = (
        df_unique_locations[df_unique_locations['ignore'] != 1])
    logging.info("Excluded locations marked with ignore flag.")

    # Extract locations from text
    doc = nlp(text)
    nlp_locations = [ent.text for ent in doc.ents if ent.label_ == "GPE"]
    logging.info(f"Extracted NLP locations: {nlp_locations}")

    # Check if any of the unique locations are in the text
    matched_locations = []
    for location in df_unique_locations['location']:
        if location.lower() in text.lower():
            matched_locations.append(location)
    logging.info(f"Matched locations: {matched_locations}")

    # If matched locations are found, return them;
    # otherwise, return NLP locations
    if matched_locations:
        logging.info("Returning matched locations.")
        return matched_locations
    else:
        logging.info("Returning NLP extracted locations.")
        return nlp_locations
...
# and address columns
# Load the unique locations data
logging.info("Loading unique locations data...")
